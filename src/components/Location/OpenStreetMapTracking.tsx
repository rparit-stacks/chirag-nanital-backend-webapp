...
const THUNDERFOREST_KEY = "64acfb2cb7254afd95277f9865cd835e";
...
const OpenStreetMapTracking: FC<OpenStreetMapTrackingProps> = ({
  ...
  const storeMarkersMapRef = useRef<Map<number, Marker>>(new Map());
  ...
  useEffect(() => {
    ...
    const initializeMap = async () => {
      ...
      const createCustomIcon = (
        ...
        if (iconType === "rider") {
          return L.divIcon({
            html: `<div class="rider-wrapper" style="width:${size}px;height:${size}px;position:relative;display:flex;align-items:center;justify-content:center;">
              <img src="/images/delivery-boy-top-view.png" class="rider-icon-inner"
                style="width:100%;height:100%;object-fit:contain;transform:rotate(${rotation}deg);transition:transform 0.2s ease-out;z-index:2;filter:drop-shadow(0 4px 10px rgba(0,0,0,0.45));" alt="rider" />
            </div>`,
            className: "",
            iconSize: [size, size],
        ...
        return L.divIcon({
          html: `<div style="background-color:${color};width:${size}px;height:${size}px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;">
            <div style="transform:rotate(45deg);color:white;font-weight:bold;font-size:${iconType === "customer" ? "20px" : "14px"};text-align:center;">${iconText}</div>
          </div>`,
      ...
      try {
        ...
        if (useTransportLayer) {
          ...
          tileLayerRef.current = L.tileLayer(
            `https://{s}.tile.thunderforest.com/transport/{z}/{x}/{y}.png?apikey=${THUNDERFOREST_KEY}`,
            {
        ...
        if (!riderMarkerRef.current) {
          let heading = 0;
          if (allSegments?.length >= 2) heading = getHeading(allSegments[0], allSegments[1]);
  ...
  useEffect(() => {
    ...
    if (Math.abs(prevPos[0] - effectiveTarget[0]) < 0.00001 && Math.abs(prevPos[1] - effectiveTarget[1]) < 0.00001) return;
      ...
      try {
        ...
        const animate = (time: number) => {
          ...
          if (riderMarkerRef.current) {
            ...
            const currentPoly = polylinesRef.current[0];
            if (currentPoly) {
              let idx = 0;
              let acc = 0;
              for (let i = 0; i < segDists.length; i++) {
              ...
              currentPoly.setLatLngs([L.latLng(currPos), ...displayRoute.slice(idx + 1).map(p => L.latLng(p))]);
            }
          }
        ...
        <div className="absolute inset-0 bg-white dark:bg-gray-900 flex items-center justify-center rounded-lg z-20">
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              <p className="text-lg font-semibold text-gray-900 dark:text-white mb-1">Loading Map</p>
              <p className="text-sm text-gray-600 dark:text-gray-400">Calculating route and markers...</p>
            </div>
          </div>
        </div>
      )}
      ...
      {!isDeliveryBoyAssigned && !isMapLoading && (
        <div className="absolute inset-0 bg-white/95 dark:bg-gray-900/95 flex items-center justify-center rounded-lg z-30 backdrop-blur-sm">
          <div className="text-center p-8 max-w-sm">
            ...
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-3">Delivery Boy Not Assigned</h3>
            <p className="text-gray-600 dark:text-gray-400 text-lg leading-relaxed">
              We are currently searching for a delivery partner. Live tracking will start once a driver is assigned.
            </p>
            <div className="mt-8">
              <div className="flex items-center justify-center gap-2">
                ...
                <span className="text-sm font-medium text-blue-600 dark:text-blue-400">Waiting for assignment...</span>
              </div>
            </div>
          </div>
        </div>
